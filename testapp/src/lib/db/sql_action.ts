...
export const test = (connection: any, callback: Function) => {
  var sql = "SELECT * FROM shape_ref";
  db.getResult(sql, connection, (err: any, results: any) => {
    // console.log(results);
    callback(err, results);
  });
};

export const getShapeRef = async (connection: any) => {
  var sql = "SELECT * FROM shape_ref";
  // console.log("connection", connection);
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
...
export const getUsers = async (connection: any) => {
...
// TODO:FIXME: my.cnf =====>>>>> default-time-zone='+9:00' 추가

// FIXME: 금일 데이터로 한정해야 한다.
export const getLastProdAssem = async (connection: any) => {
  var sql = "SELECT * FROM prod_assem ORDER BY TIME DESC LIMIT 1";
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertProdAssem = async (
  connection: any,
  model: number,
  cnt: number
) => {
  var sql = `INSERT INTO prod_assem(TIME, model, cnt) VALUES (CURRENT_TIMESTAMP, ${model}, ${cnt})`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const getLastIlAssem = async (connection: any) => {
  var sql = "SELECT * FROM il_assem ORDER BY TIME DESC LIMIT 1";
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertIlAssem = async (
  connection: any,
  model: number,
  exp1: number,
  redu: number,
  ...
  var sql = `INSERT INTO il_assem(TIME, model, EXP1, redu, oring1, EXP2, nipple, oring2) \
  VALUE (CURRENT_TIMESTAMP, ${model}, ${exp1}, ${redu}, ${oring1}, ${exp2}, ${nipple}, ${oring2})`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const getLastProdShape = async (connection: any) => {
  var sql = "SELECT * FROM prod_shape ORDER BY TIME DESC LIMIT 1";
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertProdShape = async (
  connection: any,
  model: number,
  cnt: number
) => {
  var sql = `INSERT INTO prod_shape(TIME, model, cnt) VALUES (CURRENT_TIMESTAMP, ${model}, ${cnt})`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const getLastIlShape = async (connection: any) => {
  var sql = "SELECT * FROM il_shape ORDER BY TIME DESC LIMIT 1";
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertIlShape = async (
  connection: any,
  model: number,
  cnt: number,
  beeline1: number,
  ...
  var sql = `INSERT INTO il_shape(TIME, model, cnt, beeline1, beeline2, shape1, shape2, cut1, cut2) \
  VALUE (CURRENT_TIMESTAMP, ${model}, ${cnt}, ${beeline1}, ${beeline2}, ${shape1}, ${shape2}, ${cut1}, ${cut2})`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const getLastTestLeak = async (connection: any) => {
  var sql = "SELECT * FROM test_leak ORDER BY TIME DESC LIMIT 1";
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const getTodayLeakCnt = async (connection: any) => {
  var sql = `SELECT COUNT(time) AS leak_count FROM test_leak WHERE TIME LIKE "${comm.getDate()}%"`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertTestLeak = async (
  connection: any,
  err1: number,
  err2: number,
  err3: number,
  ...
  var sql = `INSERT INTO test_leak(TIME, err1, err2, err3, err4, err5, err6) \
  VALUES (CURRENT_TIMESTAMP, ${err1}, ${err2}, ${err3}, ${err4}, ${err5}, ${err6})`;
  if (connection) {
    return await db.getResultAsync(sql, connection);
  } else {
    console.log("DB Connection Error!!!");
...
export const insertDayInit = async (connection: any, table: string) => {
  console.log("comm.getDate()", comm.getDate());
  let sql = `INSERT INTO ${table} (date, time) \
  VALUES ("${comm.getDate()}", CURRENT_TIMESTAMP) \
  ...
  db.getResult(sql, connection, function (err, results) {
    ...
    db.getResult(sql, connection, function (err, results) {
      ...
      db.getResult(sql, connection, function (err, results) {
        ...
        // );
        // SHBAEK - mariaDB
        sql = util.format("set global tx_isolation='READ-UNCOMMITTED'");
        db.getResult(sql, connection, function (err, results) {
          if (err) {
            //callback(err, results);
            //return;
...
exports.get_hit_all = function (connection, callback) {
  ...
  var sql = util.format(
    "select * from hit where ct > '" + until + "' limit 1000"
  );
  db.getResult(sql, connection, function (err, results) {
    callback(err, results);
  });
...
exports.insert_acp = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into acp (ri, pv, pvs) " + "value ('%s', '%s', '%s')",
        obj.ri,
        JSON.stringify(obj.pv).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        JSON.stringify(obj.pvs).replace(/\"/g, '\\"').replace(/\'/g, "\\'")
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.insert_ae = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, (err, results) => {
    if (!err) {
      var sql = util.format(
        "insert into ae (ri, apn, api, aei, poa, ae.or, nl, rr, csz, srv) " +
...
exports.insert_cnt = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into cnt (ri, cr, mni, mbs, mia, cni, cbs, li, cnt.or, disr) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cr,
        obj.mni,
...
exports.insert_cin = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      ...
      var sql = util.format(
        "insert into cin (ri, pi, cr, cnf, cs, cin.or, con) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.pi,
        obj.cr,
        ...
        con_type == "string"
          ? obj.con.replace(/'/g, "\\'")
          : JSON.stringify(obj.con).replace(/\"/g, '\\"').replace(/\'/g, "\\'")
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.insert_grp = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into grp (ri, cr, mt, cnm, mnm, mid, macp, mtv, csy, gn) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cr,
        obj.mt,
        obj.cnm,
        obj.mnm,
        JSON.stringify(obj.mid).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        JSON.stringify(obj.macp).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.mtv,
        obj.csy,
        obj.gn
      );
...
exports.insert_lcp = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into lcp (ri, los, lou, lot, lor, loi, lon, lost) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.los,
        obj.lou,
...
exports.insert_fcnt = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, cr) " + "value ('%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.cr
      );
...
exports.insert_hd_dooLK = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.lock, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.lock,
...
exports.insert_hd_bat = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.lvl, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.lvl,
...
exports.insert_hd_tempe = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.curT0, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.curT0,
...
exports.insert_hd_binSh = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.powerSe, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.powerSe,
...
exports.insert_hd_fauDn = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.sus, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.sus,
...
exports.insert_hd_colSn = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.colSn, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.colSn,
...
exports.insert_hd_brigs = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.brigs, cr) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.brigs,
...
exports.insert_hd_color = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into fcnt (ri, cnd, fcnt.red, fcnt.green, fcnt.blue, cr) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cnd,
        obj.red,
...
exports.insert_fwr = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mgo (ri, mgd, objs, obps, dc, vr, fwnnam, url, ud, uds) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.mgd,
        obj.objs,
        ...
        obj.url,
        obj.ud,
        JSON.stringify(obj.uds).replace(/\"/g, '\\"').replace(/\'/g, "\\'")
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.insert_bat = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mgo (ri, mgd, objs, obps, dc, btl, bts) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.mgd,
        obj.objs,
...
exports.insert_dvi = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mgo (ri, mgd, objs, obps, dc, dbl, man, mgo.mod, dty, fwv, swv, hwv) " +
...
exports.insert_dvc = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mgo (ri, mgd, objs, obps, dc, can, att, cas, cus, ena, dis) " +
        ...
        obj.can,
        obj.att,
        JSON.stringify(obj.cas).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.cus,
        obj.ena,
        obj.dis
      );
...
exports.insert_rbo = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mgo (ri, mgd, objs, obps, dc, rbo, far) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.mgd,
        obj.objs,
...
exports.insert_nod = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into nod (ri, ni, hcl, mgca) " +
          "value ('%s', '%s', '%s', '%s')",
        obj.ri,
        obj.ni,
        obj.hcl,
...
exports.insert_csr = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into csr (ri, cst, poa, cb, csi, mei, tri, rr, nl, srv) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cst,
        JSON.stringify(obj.poa).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
...
exports.insert_req = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into req (ri, op, tg, org, rid, mi, pc, rs, ors) " +
...
exports.insert_sub = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into sub (ri, pi, enc, exc, nu, gpi, nfu, bn, rl, psn, pn, nsp, ln, nct, nec, cr, su) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.pi,
        JSON.stringify(obj.enc).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.exc,
        JSON.stringify(obj.nu).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.gpi,
        obj.nfu,
        JSON.stringify(obj.bn).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.rl,
        obj.psn,
        obj.pn,
        obj.nsp,
...
exports.insert_smd = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into smd (ri, cr, dsp, dcrp, soe, rels, smd.or) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cr,
        obj.dsp,
        obj.dcrp,
        obj.soe,
        JSON.stringify(obj.rels).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.or
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.insert_ts = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into ts (ri, cr, mni, mbs, mia, cni, cbs, ts.or, pei, mdd, mdn, mdlt, mdc, mdt) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', " +
          "'%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cr,
...
exports.insert_tsi = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into tsi (ri, pi, dgt, con, sqn, cs) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.pi,
        obj.dgt,
...
exports.insert_mms = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into mms (ri, sid, soid, stid, asd, osd, sst) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.sid,
        obj.soid,
...
exports.insert_tr = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into tr (ri, cr, tid, tctl, tst, tltm, text, tct, tltp, trqp, trsp) " +
          "value ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        obj.ri,
        obj.cr,
        obj.tid,
        ...
        obj.tct,
        obj.tltp,
        JSON.stringify(obj.trqp),
        JSON.stringify(obj.trsp).replace(/\"/g, '\\"').replace(/\'/g, "\\'")
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.insert_tm = function (connection, obj, callback) {
  ...
  _this.insert_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql = util.format(
        "insert into tm (ri, tltm, text, tct, tept, tmd, tltp, tctl, tst, tmr, tmh, rqps, rsps, cr) " +
        ...
        obj.tmr,
        obj.tmh,
        JSON.stringify(obj.rqps).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        JSON.stringify(obj.rsps).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
        obj.cr
      );
      db.getResult(sql, connection, function (err, results) {
        if (!err) {
...
exports.select_resource_from_url = function (connection, ri, sri, callback) {
  var sql = util.format(
    "select * from lookup where (ri = '%s') or (sri = '%s')",
    ri,
    sri
  );
  db.getResult(sql, connection, function (err, comm_Obj) {
    if (!err) {
      if (comm_Obj.length == 0) {
        ...
        var sql =
          "select * from " +
          responder.typeRsrc[comm_Obj[0].ty] +
          " where ri = '" +
          comm_Obj[0].ri +
          "'";
...
function search_parents_lookup_action(
  connection,
  pi_list,
  count,
  cur_result_ri,
  ...
  var sql = util.format(
    "select ri, ty from lookup where pi = '" +
      pi_list[count] +
      "' and ty <> '1' and ty <> '9' and ty <> '23' and ty <> '4' and ty <> '30' and ty <> '17' limit 2000"
  );
...
exports.search_parents_lookup = function (
  connection,
  pi_list,
  cur_result_ri,
  result_ri,
...
exports.select_spec_ri = function (connection, found_Obj, count, callback) {
  ...
  var sql =
    "select * from " +
    responder.typeRsrc[found_Obj[ri].ty] +
    " where ri = '" +
    ri +
    "'";
  db.getResult(sql, connection, function (err, spec_Obj) {
...
function search_lookup_action(
  ...
  var sql = util.format(
    "select * from lookup where pi = '" + pi_list[count] + "' " + query_where
  );
  db.getResult(sql, connection, function (err, result_lookup_ri) {
    if (!err) {
      if (result_lookup_ri.length === 0) {
        search_lookup_action(
          connection,
          pi_list,
          ++count,
          result_ri,
...
function search_resource_action(
  connection,
  ri,
  query,
  cur_lim,
      ...
      if (code === "200") {
        ...
        if (query.la != null) {
          if (Object.keys(seekObj).length >= cur_lim) {
            ...
            search_resource_action(
              connection,
              ri,
              query,
              parseInt(cur_lim, 10) - foundCount,
              pi_list,
              cni,
              ++loop_count,
              seekObj,
...
var search_tid = "";
exports.search_lookup = function (
  connection,
  ri,
  query,
  cur_lim,
...
exports.select_latest_resource = function (
  connection,
  parentObj,
  loop_count,
  latestObj,
  ...
  var before_ct = moment()
    .subtract(Math.pow(5, loop_count), "minutes")
    .utc()
    .format("YYYYMMDDTHHmmss");
  var query_where =
    " and ty = '" + (parseInt(parentObj.ty, 10) + 1).toString() + "' and ";
  query_where += util.format(
    " ('%s' < ct) order by ri desc limit 10",
    before_ct
  );

  var sql =
    "select * from (select * from lookup where (pi = '" +
    parentObj.ri +
    "') " +
    query_where +
    ")b join " +
    responder.typeRsrc[parseInt(parentObj.ty, 10) + 1] +
    " as a on b.ri = a.ri";
...
exports.select_oldest_resource = function (
  connection,
  ty,
  ri,
  oldestObj,
  ...
  var sql =
    ...
    ty +
    "' limit 1)b join " +
    responder.typeRsrc[parseInt(ty, 10)] +
    " as a on b.ri = a.ri";
...
exports.select_grp_lookup = function (connection, ri, callback) {
  ...
  var sql = util.format(
    "select * from lookup where ri = '%s' and ty = '9'",
    ri
  );
    ...
    if (err) {
      ...
      if (results.length == 0) {
        ...
        if (results[0].acpi.length == 0) {
          if (results[0].ty == "3") {
            _this.select_acp_cnt(
              connection,
              ++loop,
              uri_arr,
              function (err, acpiList) {
                if (err) {
...
exports.select_acp_in = function (connection, acpiList, callback) {
  var sql = util.format(
    "select * from acp where ri in (" +
      JSON.stringify(acpiList).replace("[", "").replace("]", "") +
      ")"
  );
  db.getResult(sql, connection, function (err, results_acp) {
    callback(err, results_acp);
...
exports.select_tr = function (connection, pi, callback) {
  var sql = util.format(
    "select * from lookup where pi = '%s' and ty = '39'",
    pi
  );
  db.getResult(sql, connection, function (err, results_comm_tr) {
    if (!err) {
      if (results_comm_tr.length === 0) {
        ...
        var sql2 = util.format(
          "select * from tr where ri = '%s'",
          results_comm_tr[0].ri
        );
...
exports.select_cni_parent = function (connection, ty, pi, callback) {
  if (ty == "4") {
    var sql = util.format(
      "select cni, cbs, st, mni, mbs from cnt, lookup where cnt.ri = '%s' and lookup.ri = '%s'",
      pi,
      pi
    );
  } else {
    sql = util.format(
      "select cni, cbs, st, mni, mbs from ts, lookup where ts.ri = '%s' and lookup.ri = '%s'",
      pi,
      pi
    );
  }
...
exports.select_in_ri_list = function (
  connection,
  tbl,
  ri_list,
  ri_index,
  ...
  var sql = util.format(
    "select * from " +
      tbl +
      " where ri in (" +
      JSON.stringify(cur_ri).replace("[", "").replace("]", "") +
      ")"
  );
...
exports.select_ts_in = function (connection, ri_list, callback) {
  var sql = util.format(
    "select * from ts where ri in (" +
      JSON.stringify(ri_list).replace("[", "").replace("]", "") +
      ")"
  );
  db.getResult(sql, connection, function (err, ts_Obj) {
    callback(err, ts_Obj);
...
exports.select_count_ri = function (connection, ty, ri, callback) {
  ...
  var sql = util.format(
    "select lookup.st, %s.cni, %s.cbs FROM lookup, %s where lookup.ri = '%s' and %s.ri = '%s'",
    responder.typeRsrc[ty],
    responder.typeRsrc[ty],
    responder.typeRsrc[ty],
    ri,
    responder.typeRsrc[ty],
...
exports.update_ts_mdcn_mdl = function (connection, mdc, mdlt, ri, callback) {
  var sql = util.format(
    "update ts set mdc = '%s', mdlt = '%s' where ri = '%s'",
    mdc,
    mdlt,
    ri
  );
...
exports.update_cb_poa_csi = function (connection, poa, csi, srt, ri, callback) {
  ...
  var sql = util.format(
    "update cb set poa = '%s', csi = '%s', srt = '%s' where ri='%s'",
    poa,
    csi,
    srt,
    ri
...
exports.update_lookup = function (connection, obj, callback) {
  ...
  var sql1 = util.format(
    "update lookup set lt = '%s', acpi = '%s', et = '%s', st = '%s', lbl = '%s', at = '%s', aa = '%s', subl = '%s' where ri = '%s'",
    obj.lt,
    JSON.stringify(obj.acpi),
    obj.et,
    obj.st,
    JSON.stringify(obj.lbl).replace(/\"/g, '\\"').replace(/\'/g, "\\'"),
...
exports.update_acp = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update acp set pv = '%s', pvs = '%s' where ri = '%s'",
        JSON.stringify(obj.pv),
        JSON.stringify(obj.pvs),
        obj.ri
      );
...
exports.update_ae = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update ae set apn = '%s', poa = '%s', ae.or = '%s', rr = '%s' where ri = '%s'",
        obj.apn,
...
exports.update_cnt = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update cnt set mni = '%s', mbs = '%s', mia = '%s', li = '%s', cnt.or = '%s', cni = '%s', cbs = '%s' where ri = '%s'",
        obj.mni,
        obj.mbs,
        obj.mia,
        obj.li,
...
exports.update_grp = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update grp set mnm = '%s', mid = '%s', macp = '%s', gn = '%s' where ri = '%s'",
        obj.mnm,
        JSON.stringify(obj.mid),
        JSON.stringify(obj.macp),
        obj.gn,
        obj.ri
      );
      db.getResult(sql2, connection, function (err, results) {
...
exports.update_lcp = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update lcp set lou = '%s', lon = '%s' where ri = '%s'",
        obj.lou,
        obj.lon,
        obj.ri
      );
...
exports.update_hd_color = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update fcnt set red = '%s', green = '%s', blue = '%s'",
        obj.red,
        obj.green,
        obj.blue
      );
...
exports.update_fwr = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mgo set dc = '%s', vr = '%s', fwnnam = '%s', url = '%s', ud = '%s', uds = '%s' where ri = '%s'",
        obj.dc,
        obj.vr,
        obj.fwnnam,
        obj.url,
        obj.ud,
        JSON.stringify(obj.uds),
        obj.ri
      );
      db.getResult(sql2, connection, function (err, results) {
        if (!err) {
...
exports.update_bat = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mgo set dc = '%s', btl = '%s', bts = '%s' where ri = '%s'",
        obj.dc,
        obj.btl,
        obj.bts,
        obj.ri
...
exports.update_dvi = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mgo set dc = '%s', dbl = '%s', man = '%s', mgo.mod = '%s', dty = '%s', fwv = '%s', swv = '%s', hwv = '%s' where ri = '%s'",
        obj.dc,
        obj.dbl,
        obj.man,
        obj.mod,
...
exports.update_dvc = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mgo set dc = '%s', can = '%s', att = '%s', cas = '%s', cus = '%s', ena = '%s', dis = '%s' where ri = '%s'",
        obj.dc,
        obj.can,
        obj.att,
        JSON.stringify(obj.cas),
        obj.cus,
        obj.ena,
        obj.dis,
        obj.ri
...
exports.update_rbo = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mgo set dc = '%s', rbo = '%s', far = '%s' where ri = '%s'",
        obj.dc,
        obj.rbo,
        obj.far,
        obj.ri
...
exports.update_nod = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update nod set ni = '%s', mgca = '%s' where ri = '%s'",
        obj.ni,
        obj.mgca,
        obj.ri
      );
...
exports.update_csr = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update csr set poa = '%s', mei = '%s', tri = '%s', rr = '%s', nl = '%s' where ri = '%s'",
        JSON.stringify(obj.poa),
        obj.mei,
        obj.tri,
        obj.rr,
...
exports.update_req = function (connection, ri, pc, op, mi, rs, ors, callback) {
  ...
  var sql2 = util.format(
    "update req set pc = '%s', op = '%s', mi = '%s', rs = '%s', ors = '%s' where ri = '%s'",
    pc,
    op,
    mi,
    rs,
...
exports.update_sub = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update sub set enc = '%s', exc = '%s', nu = '%s', gpi = '%s', nfu = '%s', bn = '%s', rl = '%s', pn = '%s', nsp = '%s', ln = '%s', nct = '%s', nec = '%s' where ri = '%s'",
        JSON.stringify(obj.enc),
        obj.exc,
        JSON.stringify(obj.nu),
        obj.gpi,
        obj.nfu,
        JSON.stringify(obj.bn),
        obj.rl,
        obj.pn,
        obj.nsp,
        obj.ln,
...
exports.update_smd = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update smd set dsp = '%s', dcrp = '%s', soe = '%s', rels = '%s', smd.or = '%s' where ri = '%s'",
        obj.dsp,
        obj.dcrp,
        obj.soe,
        JSON.stringify(obj.rels),
        obj.or,
        obj.ri
      );
      db.getResult(sql2, connection, function (err, results) {
...
exports.update_ts = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update ts set mni = '%s', mbs = '%s', mia = '%s', ts.or = '%s', mdn = '%s', mdt = '%s', mdlt = '%s', mdc = '%s'  where ri = '%s'",
        obj.mni,
        obj.mbs,
        obj.mia,
        obj.or,
...
exports.update_mms = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update mms set stid = '%s', asd = '%s', osd = '%s', sst = '%s' where ri = '%s'",
        obj.stid,
        obj.asd,
        obj.osd,
        obj.sst,
...
exports.update_tm = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update tm set cr = '%s', tctl = '%s', tst = '%s', tmr = '%s', tmh = '%s', rsps = '%s' where ri = '%s'",
        obj.cr,
        obj.tctl,
        obj.tst,
        obj.tmr,
        obj.tmh,
        JSON.stringify(obj.rsps),
        obj.ri
      );
      db.getResult(sql2, connection, function (err, results) {
        if (!err) {
...
exports.update_tr = function (connection, obj, callback) {
  ...
  _this.update_lookup(connection, obj, function (err, results) {
    if (!err) {
      var sql2 = util.format(
        "update tr set cr = '%s', tctl = '%s', tst = '%s', trsp = '%s' where ri = '%s'",
        obj.cr,
        obj.tctl,
        obj.tst,
        JSON.stringify(obj.trsp),
        obj.ri
      );
      db.getResult(sql2, connection, function (err, results) {
        if (!err) {
...
exports.update_tr_trsp = function (connection, ri, tst, trsp, callback) {
  ...
  var sql2 = util.format(
    "update tr set tst = '%s', trsp = '%s' where ri = '%s'",
    tst,
    trsp,
    ri
  );
...
exports.update_cnt_cni = function (connection, obj, callback) {
  var cni_id =
    "update_cnt_cni " + obj.ri + " - " + require("shortid").generate();
  console.time(cni_id);
  var sql = util.format(
    "update cnt, lookup set cnt.cni = '%s', cnt.cbs = '%s', lookup.st = '%s' where lookup.ri = '%s' and cnt.ri = '%s'",
    obj.cni,
    obj.cbs,
    obj.st,
    obj.ri,
...
exports.update_parent_by_insert = function (connection, obj, cs, callback) {
  ...
  var cni_id =
    "update_parent_by_insert " + obj.ri + " - " + require("shortid").generate();
  console.time(cni_id);
  var sql = util.format(
    "update %s, lookup set %s.cni = %s.cni+1, %s.cbs = %s.cbs+%s, lookup.st = lookup.st+1 where lookup.ri = '%s' and %s.ri = '%s'",
    tableName,
    tableName,
    tableName,
    tableName,
...
exports.update_parent_st = function (connection, obj, callback) {
  var tableName = responder.typeRsrc[parseInt(obj.ty, 10)];
  var st_id =
    "update_parent_st " + obj.ri + " - " + require("shortid").generate();
  console.time(st_id);
  var sql = util.format(
    "update %s, lookup set lookup.st = lookup.st+1 where lookup.ri = '%s' and %s.ri = '%s'",
    tableName,
    obj.ri,
    tableName,
    obj.ri
...
exports.delete_ri_lookup_in = function (connection, ty, ri, offset, callback) {
  var sql = util.format(
    "DELETE FROM lookup WHERE pi = '%s' and ty = '%s' LIMIT %d",
    ri,
    ty,
    offset
  );
...
exports.delete_lookup = function (
  connection,
  pi_list,
  pi_index,
  found_Obj,
...
exports.delete_lookup_et = function (connection, et, callback) {
  var pi_list = [];
  var sql = util.format(
    "select ri from lookup where et < '%s' and ty <> '2' and ty <> '3' and ty <> '5'",
    et
  );
  db.getResult(sql, connection, function (err, delete_Obj) {
    if (!err) {
