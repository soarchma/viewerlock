...
function main() {
  ...
  echo "main()----------------BOLT_PATH: ${BOLT_PATH}"
  echo "main()----------------ENGINE_PATH: ${ENGINE_PATH}"
  ...
  # echo "$(id -u)"
  # exec ls -al ${BOLT_PATH}

  if [ "$(id -u)" = 0 ]; then
    echo "HAHAHAHAHAHAHAHAHAHAHAHAHAHAHA"
    exec su-exec influxdb "$BASH_SOURCE" "${@}"
    return
  fi

  echo "KEKEKEKEKEKEKEKEKEKEKEKEKEKEKEKEK"
  # Run influxd.
  exec influxd "${@}"
}
